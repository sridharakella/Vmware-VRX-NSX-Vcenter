...
//#--------------------------------------------------------#
  ...
  //#
  //# [Info] 
  //#   - Action to run any scripts to any OS on any Cloud Account Provider
  //#   - Action features:
  ...
  //#      - Run script regardless in which cloud account we deploy to: Amazon Web Services (AWS), Google 
  //#        Cloud Platform (GCP) (under development), Microsoft Azure, VMware vCenter, VMC
  ...
  //#   - For Powershell only Basic auth is supported. Requires following winrm config: 
  ...
  //#   - Condition Filter: 
  //#      - In the Subscription condition filter create a filter for each cloud account and map it 
  //#        to the corresponding action . E.g. for the Subscription that will wall the action which 
  //#        is configured fort Azure, we will add the following condition filter 
  //#        event.data.customProperties.abxRunScript_endpointType == "azure". For the Subscription 
  //#        that will trigger the ABX action that is configured to run on AWS FaaS we will add the 
  //#        filter event.data.customProperties.abxRunScript_endpointType == "aws", ect..
  //# [Bluepirnt]
...
exports.handler = function handler(context, inputs) {
    ...
    var psUser = inputs.psUserIn;     // PS Username
    var psPass = inputs.psPassIn;     // PS Password
...
-----END RSA PRIVATE KEY-----`;
    ...
    if (cmdShellType == "linux".toLowerCase()){       // Loop to check if SHELL or POWESHELL will be run
        ...
        if ((actionOptionAllowBlueprintScripts == "true".toLowerCase()) && (inputs.customProperties !== undefined) ) {      // Loop to check if BP Script is allowed and run it
            ...
            winrm.runCommand(cmdBpScript , cmdHost , psUser , psPass , psPort);      // Script Bluerpint Run 
